...
const StoryboardModule = {
  ...
  async _generateShots() {
    ...
    // 2. 构建 Prompt
    const systemPrompt = `你是一位顶级的影视分镜师。以下是你必须严格遵守的输出规则。

输出一个JSON数组，每个元素代表一个分镜镜头。

每个镜头必须包含以下字段：
- shotNumber: 镜头编号（数字，从1开始）
- sceneType: 景别（远景/全景/中景/近景/特写/大特写/中近景/手机特写等）
...
- mood: 情绪标签
- soundEffect: 音效描述
- description: （画面）详细描述——画面构图、人物位置、光影色调、特效动画等
- prompt: 提示词字段。格式必须严格如下，每个【】字段单独一行：
  【场景】当前镜头所在的具体场景描述
  【角色】镜头中出现哪些角色
  【景别】远景/全景/中景/近景/特写/大特写等
  【镜头角度】平拍/俯拍/仰拍/侧拍/过肩镜头/主观视角等
  【镜头时长】该镜头持续的秒数
  【焦点】人物/场景/道具/动作
  【画面】详细的画面描述，包括构图、人物位置、光影、色调等
  【动作】预备→过程→终点——描述动作三阶段的完整细节
  【表情控制】角色面部表情的具体要求
  【面部微动作】角色细微的面部动作（眼神、嘴角、眉毛等）
  【镜头运动】固定镜头/推镜/拉镜/摇镜/移镜/跟镜/升降等
  【台词】角色说出的具体台词
  【音效】环境音效和特效音

注意：每个【】字段必须都有内容，不能为空。prompt字段要完整、专业、可直接用于AI视频生成。

重要创作原则：
1. 按场景切换来组织镜头，不同场景之间自然过渡
2. 景别要丰富变化，不要连续使用相同景别
3. 镜头运动要匹配情绪：压抑用慢镜头，紧张用推镜，对话用固定镜头
4. 画面描述要包含具体的视觉细节（光影、色调、构图）
5. 音效要具体（如有特殊处理也要写清楚）
6. 确保覆盖剧本全部内容，不要遗漏关键情节
