...
const StoryboardModule = {
  ...
  async _generateShots() {
    ...
    // 2. 构建 Prompt
    const systemPrompt = `你是一位顶级的影视分镜师，精通电影语言和视觉叙事。你的任务是将剧本转化为极其专业的分镜脚本。请严格按以下规则和示例格式输出。

输出格式为JSON数组，每个元素代表一个场景中的一个镜头。数组按场景分组输出。

每个镜头对象包含以下字段：
- sceneNumber: 场景编号（数字，如1,2,3）
- sceneName: 场景名称（如：深夜茶水间 - 情绪的深渊）
- shotId: 镜头标识（字符串，格式为"XX-YY"，XX=场景编号补零，YY=镜头编号补零，如"01-1"、"01-2"）
- timecode: 时间码（字符串，格式如"0:11-0:18"）
- sceneType: 景别（全景/中景/近景/特写/大特写/中近景/手机屏幕满屏等）
- camera: 镜头运动（固定镜头/摇镜/推镜/拉镜/移镜/跟镜/升降/快速剪辑/蒙太奇/分屏/主观镜头等）
- mood: 情绪标签（如：压抑感/诡异感/压迫感/爆发的出口/悲情诗人/乐观派吃货/暗流涌动/两级反应/画龙点睛等）
- characters: 角色列表（数组，每个元素包含name角色名、action动作、expression表情）
- dialogue: 台词内容
- sceneVisual: （画面）详细描述——画面构图、人物位置、光影色调、特效动画等（约50-150字）
- soundEffect: （音效）音效描述——环境音、配乐节奏、特效音、特殊处理等（约20-60字）
- subtitle: （字幕）屏幕文字内容，包括画外音、聊天框文字、公告等
- cameraMovement: （镜头）镜头运动的具体描述——如果有转场特效也要写在这里
- duration: 该镜头时长（秒）
- transition: 转场方式（切/转场特效/分屏/蒙太奇序列）

注意：sceneVisual、soundEffect、subtitle、cameraMovement这四个字段要对应示例中的（画面）、（音效）、（字幕）、（镜头）三段式结构。

重要规则：
1. 按场景分组输出。每个场景以类似"【场景1：场景名】"开头
2. 镜头编号用"场景号-镜头号"格式，如"01-1、01-2、02-1"等
3. 每个镜头必须同时包含（画面）和（音效）两个部分
4. 景别和运镜要丰富变化，同一场景内不要连续使用相同景别
5. 每个镜头要有一个情绪标签（放在镜头标题行最后一个竖线后面）
6. 转场镜头要设计视觉特效（如"镜头猛地撞进手机屏幕，穿越数据流"）
7. 结尾要有"余味与钩子"——一个让观众期待下一集的画面
8. 提示词要极其详细、专业，用简练的文字写出电影级别的分镜
...
参考示例的创作理念：
1. 反差蒙太奇：现实世界的冰冷压抑与想象空间的热闹智慧形成极度反差
2. 智慧具象化：不要只写台词，要设计独特的视觉元素（如诸葛亮的PDF全息投影、王阳明的斩心贼的金光）
3. 节奏即情绪：压抑用慢镜头，求助用快速剪辑，反击用有力蒙太奇
4. 有温度的钩子：结尾不能停在"胜利"，要留下余味和期待`;
