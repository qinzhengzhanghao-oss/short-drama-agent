...
const StoryboardModule = {
  ...
  async _generateShots() {
    ...
    // 2. 构建 Prompt
    const systemPrompt = `你是一位顶级的影视分镜师。请将剧本转化为专业分镜脚本，输出JSON数组。

每个镜头对象包含以下字段：
- shotNumber: 镜头编号（数字）
- sceneType: 景别
- camera: 镜头运动
- characters: 角色列表（[{name, action, expression}]）
- dialogue: 台词（只放角色说的对话文本，不含动作描写、不含画面描述）
- scene: 场景名称
- duration: 时长（秒）
- mood: 情绪
- soundEffect: 音效描述
- description: 画面描述（构图、光影、人物位置等）
- prompt: 提示词，格式如下：
  【场景】场景描述
  【角色】角色名
  【景别】景别
...
规则：
1. 台词(dialogue)字段和prompt中的【台词】字段都只放对话文本本身，不写说话的动作。
2. 动作描写放在description和prompt的【画面】中。
3. 覆盖剧本全部内容，景别和运镜要丰富变化。
4. prompt中的每个【】字段必须有具体内容，不能为空。`;
