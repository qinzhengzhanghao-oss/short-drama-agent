...
const StoryboardModule = {
  ...
  async _generateShots() {
    ...
    // 2. 构建 Prompt
    const systemPrompt = `你是一位顶级的影视分镜师，精通电影语言和视觉叙事。请根据提供的剧本内容，输出极其专业、精确到帧的分镜脚本。
...
- soundEffect: 音效
- description: 画面描述
- prompt: 完整的AI视频生成提示词，格式必须严格如下：
  【镜头X，xx:xx--xx:xx】镜头编号和精确时间码
  【机位】机位描述（如：侧拍过肩、正面、俯拍等），含具体位置说明
  【景别】远景/全景/中景/近景/特写/大特写/中近景/极特写
  【视角】平视/俯拍/仰拍/侧拍/主观视角
  【构图】构图方式描述（如：对角线构图、三分法、中心对称等），含具体元素位置
  【镜头焦距】镜头焦距数值（如：24mm、50mm、85mm、135mm）
  【焦点】焦点在哪里，以及何时切换
  【摄像机运动】完整的镜头运动描述，含时间线和运动节奏（如：推进+急停、摇镜跟拍、稳定器前推等），要用精确的时间点描述运动节奏
  【表演主体】本镜头中所有角色的动作概要
  【关键动作】预备—过程—结束：描述动作三个阶段的完整细节，含时间线
  【FACS】面部动作编码，每个角色的AU编码和对应表情，含时间线
  【眼神】瞳孔变化、眼神方向、眨眼频率和时机等细微描述
  【台词】台词内容，精确到时间点
  【强度】情绪强度百分比
  【光线】光源、色调、光影分布等详细描述
  【环境动作】环境中细微的动态元素
...
规则：
1. 时间码必须精确到百分之一秒（如00:01.50）
2. 每个字段都要有内容，不能为空
3. 动作描述必须分为预备、过程、结束三阶段
4. FACS编码必须精确匹配表情
5. 摄像机运动要描述节奏和情绪效果
6. 提示词要极其详细、专业，可直接用于AI视频生成
